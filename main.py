...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--hidden",         type=int,   default=None)
    p.add_argument("--out",            type=int,   default=None)
    p.add_argument("--struct-proj",    type=int,   default=None,
                   help="Structured-feature projection dim (0 = disable)")
    p.add_argument("--device",         type=str,   default=None)
    return p.parse_args()
...
def main():
    ...
    if args.hidden:       gnn_cfg.hidden_channels        = args.hidden
    if args.out:          gnn_cfg.out_channels           = args.out
    if args.struct_proj is not None:
                          gnn_cfg.structured_proj_dim    = args.struct_proj
    if args.device:       train_cfg.device               = args.device
    ...
    model = HeteroGraphSAGE(
        ...
        dropout              = gnn_cfg.dropout,
        metadata             = data.metadata(),
        structured_proj_dim  = gnn_cfg.structured_proj_dim,
    )
    n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    log.info("Model parameters: %d", n_params)
