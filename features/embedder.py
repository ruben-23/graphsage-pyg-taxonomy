...
from config.settings import OLLAMA_BASE_URL, OLLAMA_MODEL, RAW_EMBEDDING_DIM
    ...
    def _embed_single_with_retry(self, text: str) -> List[float]:
        for attempt in range(self.max_retries):
            try:
                ...
                log.warning("Empty or invalid embedding format for text snippet")
                return [0.0] * self._dim
            except Exception as exc:
                log.warning(
                    "Ollama attempt %d/%d failed: %s",
